...
import { TrendingUp, AlertTriangle, Loader2, X, Sparkles, Layers, Info } from 'lucide-react';
...
export interface MetaAnalysisPanelProps {
  ...
  isMobile?: boolean;
  activeRightTab?: 'detail' | 'meta';
  setActiveRightTab?: (tab: 'detail' | 'meta') => void;
    ...
    >
      ...
      {!rightPanelOpen && !isMobile ? (
        ...
        <div className="flex-1 flex items-center justify-center cursor-pointer" onClick={() => setRightPanelOpen(true)}>
          ...
          >
            {currentTab === 'detail' ? 'Detalle' : 'Análisis'}
          </span>
        </div>
      ) : (
